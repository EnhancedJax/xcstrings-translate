...
import {
  ...
  DEFAULT_AUTO_TRANSLATION_MODEL,
  DEFAULT_CHUNK_SIZE,
  TEMP_SOURCE_FILE_PREFIX,
  TEMP_TRANSLATED_FILE_PREFIX,
...
export interface CliDependencies {
  runCopilotTranslationJob?: (
    prompt: string,
    model: string,
  ) => Promise<CopilotJobResult>;
  now?: () => number;
  pid?: number;
}
  ...
  for (const optionName of listProvidedOptions(args)) {
    if (!allowedOptions.has(optionName)) {
      throw new XcstringsError(
        "INVALID_ARGUMENT",
        `Option ${optionName} is not valid for this workflow.`,
      );
    }
  }
}
...
function buildSourceCsvPath(
  ...
  return path.join(
    outDir,
    `${TEMP_SOURCE_FILE_PREFIX}_${locale}_${runToken}.csv`,
  );
}

function buildTranslatedCsvPath(
  outDir: string,
  locale: string,
  runToken: string,
  index: number,
  ...
  return path.join(
    outDir,
    `${TEMP_TRANSLATED_FILE_PREFIX}_${locale}_${runToken}_${index}.csv`,
  );
}
...
function validateTranslatedCsvFile(
  filePath: string,
  locale: string,
...
async function runAgentTranslationWithRetries(
  ...
  const runner =
    dependencies.runCopilotTranslationJob ?? runCopilotTranslationJob;

  for (let attempt = 1; attempt <= totalAttempts; attempt += 1) {
    const startedAt = Date.now();
    console.error(
      `Agent ${task.label}: Working... (attempt ${attempt}/${totalAttempts})`,
    );
    ...
    const validation = validateTranslatedCsvFile(
      task.outCsvPath,
      task.targetLocale,
    );
    if (result.exitCode === 0 && validation.ok) {
      const elapsedSeconds = Math.floor((Date.now() - startedAt) / 1000);
      console.error(
        `Agent ${task.label}: Done in ${elapsedSeconds}s -> ${task.outCsvPath}`,
      );
      return;
    }

    const reason =
      validation.reason ?? `copilot exited with code ${result.exitCode}`;
...
function runImportOnlyWorkflow(
  catalogPath: string,
  args: ReturnType<typeof parseStandaloneArgs>,
): void {
  ...
  if (importPath == null) {
    throw new XcstringsError(
      "INVALID_ARGUMENT",
      "--import requires a CSV path value.",
    );
  }
...
function runCcWorkflow(
  ...
  if (ccValue == null) {
    throw new XcstringsError(
      "INVALID_ARGUMENT",
      "--cc requires a locale value (zh-Hans or zh-Hant).",
    );
  }
  ...
  if (ccValue === "zh-Hans") {
    ...
    throw new XcstringsError(
      "INVALID_ARGUMENT",
      "--cc must be one of: zh-Hans, zh-Hant.",
    );
  }
  ...
  console.error(
    `Skipped (missing ${sourceLocale}) ${result.skippedMissingSource} key(s)`,
  );
  console.error(
    `Skipped (already had ${targetLocale}) ${result.skippedAlreadyTranslated} key(s)`,
  );
  console.error(
    `Skipped (shouldTranslate=false) ${result.skippedNotTranslatable} key(s)`,
  );
}
...
async function runAutoWorkflow(
  ...
  if (autoState.value == null) {
    throw new XcstringsError(
      "INVALID_ARGUMENT",
      "--auto requires a comma-separated locale list.",
    );
  }

  const locales = parseLocaleList(autoState.value, "--auto");
  const outDir = ensureOutputDirectory(
    getOptionValue(args, "--out") ?? tmpdir(),
  );
  const chunkSize =
    parsePositiveIntOption(args, "--chunk-size") ?? DEFAULT_CHUNK_SIZE;
  ...
  const runToken = `${(dependencies.now ?? Date.now)()}_${dependencies.pid ?? process.pid}`;
  ...
  for (const locale of locales) {
    ...
    for (const sourceCsvPath of exportResult.writtenCsvPaths) {
      tasks.push({
        sourceCsvPath,
        outCsvPath: buildTranslatedCsvPath(
          outDir,
          locale,
          runToken,
          tasks.length + 1,
  ...
  await Promise.all(
    tasks.map(async (task) => {
      try {
        await runAgentTranslationWithRetries(task, model, dependencies);
        ...
        completedCount += 1;
        console.error(
          `Auto mode status: ${completedCount - failedCount} done, ${failedCount} failed.`,
        );
      }
    }),
  );
  ...
  for (const translatedCsvPath of translatedCsvPaths) {
    ...
    totalSkippedAlreadyTranslated +=
      importResult.skippedAlreadyTranslated.length;
    totalWarnings += importResult.warnings.length;

    console.error(
      `Imported ${translatedCsvPath}: updated ${importResult.updated}`,
    );
  }
...
export async function runCli(
  ...
  try {
    ...
    const selectedWorkflows = [hasImport, hasCc, hasAuto].filter(
      Boolean,
    ).length;
    ...
    await runAutoWorkflow(catalogPath, args, dependencies);
    return 0;
  } catch (error) {
    printError(error);
    return 1;
